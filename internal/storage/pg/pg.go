...
func NewPostgres(ctx context.Context, logger *slog.Logger, pgURL string, redisClient *redis.Redis) (*Postgres, error) {
	if pgURL == "" {
...
func (p *Postgres) GetByUID(ctx context.Context, id int) (domain.Order, error) {
	var o domain.Order
...
func (p *Postgres) CreateOrder(ctx context.Context, o domain.Order) (int, error) {
...
func (p *Postgres) GetAllOrderIDs(ctx context.Context) ([]domain.Order, error) {
	...
	query := `
        SELECT 
            ...
            p.Transaction, p.Currency, p.Provider, p.Amount, p.PaymentDt, p.Bank, p.DeliveryCost, p.GoodsTotal,
            d.Name, d.Phone, d.Zip, d.City, d.Address, d.Region, d.Email
        FROM orders o
        JOIN payment p ON o.payment_id_fk = p.id
        JOIN delivery d ON o.delivery_id_fk = d.id
    `

	rows, err := p.pool.Query(ctx, query)
	if err != nil {
		return nil, e.Wrap("failed to query getAllOrders with payment and delivery", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err != nil {
			return nil, e.Wrap("failed to scan order with payment and delivery", err)
		}
	...
	if err := rows.Err(); err != nil {
		return nil, e.Wrap("error during row iteration", err)
	}
